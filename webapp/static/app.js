...
function status(state, title, detail) {
  $("submission-status").dataset.state = state;
  $("status-title").textContent = title;
  $("status-detail").textContent = detail;
}
function download(value, name) {
  const a = document.createElement("a");
  a.href = `data:application/cose;base64,${value.replace(/-/g, "+").replace(/_/g, "/")}`;
  a.download = name;
  a.click();
}
function party(role) {
  return config.parties.find((item) => item.role === role).path;
}
const optional = (id) => $(id).value.trim() || undefined;
function report() {
  const date = $("patch-date").value,
    refs = $("references")
      .value.split(/\r?\n/)
      .map((v) => v.trim())
      .filter(Boolean);
...
async function signer() {
  ...
  const file = $("issuer-key").files[0];
  if (!file) throw new Error("select a PKCS#8 PEM or private P-256 JWK");
...
$("composer").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const issued = await issueReport(
      ...
      report(),
      config.msrcJwk,
      (jwk) => post(party("issuer"), { public_jwk: jwk }),
      await signer(),
    );
    ...
    status(
      "success",
      "Submission complete",
      `Registered in ${config.ledger.name} and delivered to MSRC`,
    );
    artifacts = { redacted: b64(issued.token), full: b64(statement) };
    $("txid").textContent = delivered.txid;
    $("redacted-size").textContent = `${issued.token.length.toLocaleString()} B`;
    $("full-size").textContent = `${statement.length.toLocaleString()} B`;
...
document.querySelectorAll('[name="key-mode"]').forEach((input) => {
  input.onchange = () => {
    ...
    $("issuer-key").disabled = !upload;
    $("key-state").textContent = upload ? "Select a private P-256 key" : "Generated when submitted";
  };
});
$("issuer-key").onchange = () =>
  ($("key-state").textContent = $("issuer-key").files[0]?.name || "Select a private key.");
$("reset").onclick = () => {
  for (const [id, value] of Object.entries(defaults))
    $(id === "patchDate" ? "patch-date" : id).value = value;
};
