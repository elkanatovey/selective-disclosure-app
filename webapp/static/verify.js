...
function renderReport(report) {
  ...
  else $("references").append(field("", null).lastChild);
}
function renderChecks(result) {
  ...
  $("overall").className = `overall ${result.valid ? "pass" : "fail"}`;
  $("overall-title").textContent = result.valid ? "Disclosure verified" : "Verification failed";
  const limited = result.checks.some((check) => check.status === "unavailable");
  $("overall-detail").textContent = result.valid
    ? limited
      ? "Valid under the prototype trust model"
      : "All checks passed"
    : "Do not rely on disclosed contents";
  lucide.createIcons();
}
async function load(file) {
  token = new Uint8Array(await file.arrayBuffer());
  $("file-title").textContent = file.name;
  $("file-detail").textContent = `${token.length.toLocaleString()} B`;
  $("verify").disabled = false;
  $("results").hidden = true;
  $("verify-error").textContent = "";
}
$("token-file").onchange = (event) => load(event.target.files[0]);
...
$("verify-form").onsubmit = async (event) => {
  ...
  try {
    ...
    $("verify-error").textContent = "";
    const result = await verify(token, $("audience").value);
...
$("verify-another").onclick = () => {
  $("results").hidden = true;
  $("verify-form").hidden = false;
  $("token-file").value = "";
  token = undefined;
  $("verify").disabled = true;
  $("file-title").textContent = "Choose a signed disclosure";
  $("file-detail").textContent = "Load a `.kbt.cose` file";
};
lucide.createIcons();
