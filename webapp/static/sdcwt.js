...
const head = (major, n) => {
  ...
  if (n <= 255) return Uint8Array.of((major << 5) | 24, n);
  if (n <= 65535) return Uint8Array.of((major << 5) | 25, n >> 8, n);
  if (n <= 0xffffffff) return Uint8Array.of((major << 5) | 26, n >>> 24, n >>> 16, n >>> 8, n);
...
export function decode(bytes) {
  ...
  const read = () => {
    ...
    if (major === 7) {
      ...
      if (ai === 24) return new Simple(take(1)[0]);
    ...
    if (major === 3) return text.decode(take(n));
    ...
    if (major === 6) return new Tag(n, read());
...
const random = () => crypto.getRandomValues(new Uint8Array(16)),
  hash = async (bytes) => new Uint8Array(await crypto.subtle.digest("SHA-256", bytes));
...
const chunks = (value) => {
  const chars = Array.from(value.replace(/\r\n?/g, "\n").normalize("NFC")),
    out = [];
  for (let i = 0; i < chars.length; i += 6) out.push(chars.slice(i, i + 6).join(""));
  return out;
};
...
function check(report) {
  const allowed = new Set(FIELDS.slice(1).map(([name]) => name));
  for (const name of Object.keys(report))
    if (!allowed.has(name)) throw new TypeError(`unknown report field: ${name}`);
  for (const name of ["title", "body", "component", "severity", "patch"])
    if (report[name] != null && typeof report[name] !== "string")
      throw new TypeError(`${name} must be text`);
  if (report.patch_date != null && !Number.isSafeInteger(report.patch_date))
    throw new TypeError("patch_date must be an integer");
  if (
    report.references != null &&
    (!Array.isArray(report.references) ||
      report.references.some((item) => typeof item !== "string"))
  )
    throw new TypeError("references must be an array of text");
}
async function redact(report) {
  ...
  for (const [name, key] of FIELDS) {
    ...
    if (name === "parent" || value == null) value = random();
    else if (name === "body") {
      const nested = [];
      for (const [index, chunk] of chunks(value).entries()) {
        const opening = encode([random(), chunk, index]);
        disclosures.push(opening);
        nested.push(await digest(opening));
      }
      value = new Map([[RCK, nested.sort(compare)]]);
    } else if (name === "fingerprint") value = hex(value);
    else if (name === "references") {
      const nested = [];
      for (const item of value) {
        const opening = encode([random(), item]);
        disclosures.push(opening);
        nested.push(new Tag(60, await digest(opening)));
      }
      value = nested;
    }
    const opening = encode([random(), value, key]);
    disclosures.push(opening);
...
export async function inspectStatement(statement) {
  ...
  for (const hash of payload.get(root)) {
    ...
    const [, value, key] = opening.value,
      field = {
        key,
        name: FIELDS.find(([, id]) => id === key)?.[0],
        opening: opening.encoded,
        value,
        children: [],
      };
    if (value instanceof Map) {
      const nested = redKey(value);
      for (const hash of value.get(nested) || []) {
      ...
      field.children.sort((a, b) => a.index - b.index);
    } else if (Array.isArray(value)) {
      for (const [index, item] of value.entries())
        if (item instanceof Tag && item.tag === 60) {
  ...
  if (fields.size !== 9) throw new Error("statement does not contain the report schema");
...
export async function signKbt(model, selected, signer, audience) {
  ...
  if (!selected.length) throw new TypeError("select at least one disclosure");
  const cnf = model.payload.get(8)?.get(1),
    same = (left, right) => compare(left, right) === 0;
  if (
    !(cnf instanceof Map) ||
    !same(cnf.get(-2), unb64(signer.publicJwk.x)) ||
    !same(cnf.get(-3), unb64(signer.publicJwk.y))
  )
    throw new Error("signing key does not match the statement cnf");
  const tagged = decode(model.statement),
    parts = tagged.value,
    headers = new Map(parts[1]);
  headers.set(17, selected);
  const presented = new Tag(18, [parts[0], headers, parts[2], parts[3]]),
    ...
    toSign = encode(["Signature1", protectedBytes, new Uint8Array(), payloadBytes]),
    signature = new Uint8Array(
      await crypto.subtle.sign({ name: "ECDSA", hash: "SHA-256" }, signer.privateKey, toSign),
    );
