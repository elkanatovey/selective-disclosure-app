...
if(!mode||!output)throw new Error("usage: scitt_flow.mjs issue|issue-foreign|present OUTPUT [APP_URL]");
...
if(mode==="issue"||mode==="issue-foreign"){
  ...
  const state=await json(`${appUrl}/api/state`),holderPath=state.parties.find(item=>item.role==="holder").path,msrcOrigin=new URL(holderPath,appUrl).origin,selected=[title.opening,body.opening,body.children[0].opening,references.opening,references.children[0].opening];
  await postCose(`${msrcOrigin}/api/inspect`,presented);
  const kbt=await postForCose(`${msrcOrigin}/api/disclosures`,{statement:b64(presented),selected:selected.map(b64),audience:metadata.audience});
