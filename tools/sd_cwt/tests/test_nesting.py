...
def test_deeply_nested_map_and_array_path(signer):
    ...
    token, discs = sd_cwt.issue(
        claims,
        [(700, "a", "b", 1)],  # redact the element "secretY"
        signer,
    )
...
def test_issue_rejects_unresolved_redaction_paths(signer, redact_paths):
    claims = {502: ["a", "b"], 700: {"items": ["x"], "scalar": "value"}}

    with pytest.raises(ValueError):
        sd_cwt.issue(claims, redact_paths, signer)
